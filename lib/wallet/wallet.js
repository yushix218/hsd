...
class Wallet extends EventEmitter {
  ...
  async _renameAccount(acct, name) {
    ...
    // TODO(boymanjor): decide if this matters for watch-only
    // wallets. If so, we'll need to check a different attribute,
    // i.e. id or a new, monotonically increasing field. Leaning
    // towards replacing with account.accountDepth.
  ...
  async _createAccount(options, passphrase) {
    ...
    const opt = {
      wid: this.wid,
      // TODO(boymanjor): decide if this suffices as the object to use
      // to replace the uniqueness constraint on accountIndex or if we
      // should add accountDepth as another attribute. Leaning towards
      // adding accountDepth.
      id: this.id,
      // TODO(boymanjor): decide if 'default' account even makes
      // sense for watch only wallets. We have to provide an account
      // key. If so, this is fine. If not, we need to move this logic
      // to the if-else block above.
  ...
  // TODO(boymanjor): currently this only allows a string
  // as input. This would imply a user can only use the
  // name from the API. This might be fine. We will need
  // to update this function to use accoundDepth instead
  // of accountIndex.
  ...
  // TODO(boymanjor): all occurrences of this function will
  // need to be replaced with getAccountDepth. This function
  // will need to accept a name.
  //
  // TODO(boymanjor): we need to create another function called
  // getAccountDepth.
  ...
  // TODO(boymanjor): this needs to be replaced with
  // ensureDepth, or we can keep this function and
  // add ensureDepth.
  //
  // All calls to ensureIndex need to be replaced with
  // ensureDepth.
  ...
  // TODO(boymanjor): this function needs to be updated
  // to use name or accountDepth.
  ...
  async getWalletCoinView(tx, view) {
    ...
    for (const input of tx.inputs) {
      ...
      //
      // TODO(boymanjor): remove when bug is fixed.
  ...
  // TODO(boymanjor): this needs to be updated to
  // accept the accountDepth instead of accountIndex.
  async _zap(acct, age) {
