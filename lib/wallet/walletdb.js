...
class WalletDB extends EventEmitter {
  ...
  renameAccount(b, account, name) {
    ...
    // TODO(boymanjor): this needs to be updated
    // to account.id or account.accountDepth
  ...
  // TODO(boymanjor): this function needs to accept
  // accountDepth as its second argument.
  async getAccount(wid, index) {
    ...
    // TODO(boymanjor): this needs to be updated to store
    // accountDepth and add the accountDepth to the account
    // object. We will need to store the index under another key.
  ...
  // to look under a different key.
  //
  // TODO(boymanjor): a new function named getAccountDepth
  // needs to be written that mimics this functionality.
  ...
  // TODO(boymanjor): this function needs to be updated
  // to look under another key and use accountDepth.
  ...
  saveAccount(b, account) {
    const wid = account.wid;
    // TODO(boymanjor): we need to also grab accountDepth.
    const index = account.accountIndex;
    const name = account.name;

    // TODO(boymanjor): this will need to be updated to
    // to store the accountDepth instead of index.
    // Account data
    b.put(layout.a.encode(wid, index), account.encode());

    // TODO(boymanjor): this needs to be updated to store
    // the accountDepth instead of index.
    //
    // TODO(boymanjor): we need another record type that
    // stores account depth instead of account index.
    // We should keep record type as well.
    // Name->Index lookups
    b.put(layout.i.encode(wid, name), fromU32(index));

    // TODO(boymanjor): this needs to be updated to store
    // accountDepth instead of index.
    // Index->Name lookups
    b.put(layout.n.encode(wid, index), fromString(name));
  }
  ...
  // TODO(boymanjor): this function needs to to look
  // under a different key and use accountDepth.
