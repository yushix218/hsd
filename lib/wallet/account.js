...
class Account extends bio.Struct {
  ...
  fromOptions(options) {
    ...
    // TODO(boymanjor): watch-only wallets can have multiple
    // accounts share an index. We need to use a different
    // mechanism for automatic naming. Perhaps add accountDepth
    // to the account and use this? Perhaps require the name to
    // be passed...
    if (!this.name)
